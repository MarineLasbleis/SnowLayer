...
# Load the profiles PREM and AK135. radius in m, alpha in m/s, Ks in Pa. 
    ...
    Kprem_labrosse2015=(param.K0-param.K0*param.Kprim0*(ric**2./param.Lrho**2+4./5.*ric**4./param.Lrho**4))
    rho_Labrosse2015=param.rho0*(1-ric**2/param.Lrho**2-param.Arho*ric**4/param.Lrho**4)
    K_labrosse2003=1777e9*rho_Labrosse2015/7.5e3*(np.log(rho_Labrosse2015/7.5e3)+1)
    K_approx=KPREM[0]-(KPREM[1]-KPREM[0])/(radius_PREM[1]-radius_PREM[0])*radius_PREM[0]+(KPREM[1]-KPREM[0])/(radius_PREM[1]-radius_PREM[0])*ric
    K_approx=K_approx*1.e8


    DATA_PREM={'r':r_PREM.tolist(), 'Vp':alpha_PREM.tolist(), 'K':KPREM.tolist(), 'hmin':hminP+1, 'hmax':hmaxP+1, 'rho': rho_PREM.tolist()}
    f=open("F_layer_PREM.dat",'w')
    json.dump(DATA_PREM,f)
    f.close()
    DATA_PREM2={'r':r_PREM2.tolist(), 'Vp':alpha_PREM2.tolist(), 'K':K_PREM2.tolist(),'hmin':20, 'hmax':30,'rho': rho_PREM2.tolist()}
    f=open("F_layer_PREM2.dat",'w')
    json.dump(DATA_PREM2,f)
    f.close()
    DATA_AK135={'r':r_AK.tolist(), 'Vp':alpha_AK.tolist(), 'hmin':hminA-1, 'K':K_AK.tolist(), 'hmax':hmaxA-1, 'rho': rho_AK.tolist()}
    f=open("F_layer_AK135.dat",'w')
    json.dump(DATA_AK135,f)
    f.close()
    Vp_Labrosse=np.sqrt(Kprem_labrosse2015/rho_Labrosse2015)
    DATA_Labrosse={'r':ric.tolist(), 'Vp':Vp_Labrosse.tolist(), 'hmin':1, 'K':Kprem_labrosse2015.tolist(), 'hmax':30, 'rho': rho_Labrosse2015.tolist()}
